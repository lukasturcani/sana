...
class LocalFilePicker(ui.dialog):
    def __init__(
        ...
        directory: str,
        *,
        upper_limit: str | None = UNDEFINED,
        multiple: bool = False,
        show_hidden_files: bool = False,
    ) -> None:
        """Local File Picker.

        This is a simple file picker that allows you to select a file from the
        local filesystem where NiceGUI is running.

        Parameters:
            directory: The directory to start in.
            upper_limit:
                The directory to stop at. If None then no limit if default
                then same as the starting directory.
        ...
        if upper_limit is None:
            ...
            self.upper_limit = Path(
                directory if upper_limit is UNDEFINED else upper_limit
            ).expanduser()
